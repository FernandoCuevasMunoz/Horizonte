...
| 41 | 22 Jul | Modal Video/Fotos + parking icon | Modal unificado con pills [Fotos] [Video] que abren un lightbox con tabs para cambiar entre vistas sin cerrar. Pills y X posicionados relativos al contenido (no al viewport). Icono `CircleParking` reemplaza `Car`. Parking muestra solo el número (sin "E"). Video solo visible en modal, no inline. |
