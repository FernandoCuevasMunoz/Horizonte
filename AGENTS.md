...
| 39 | 7 Jul | @Async + Contacto tarjetas + Status toggle Vendido/Arrendado | `@EnableAsync` + `@Async` en `ContactEmailService` para envío asíncrono sin trabar el botón; página Contacto con tarjetas de Felipe Figueroa (+56993001522) y Fernando Cuevas (+56944938291); campo `status` (Vendido/Arrendado) en `Property.java` con toggle contextual en admin según operation, overlay grayscale + badge en `PropertyCard` y `PropertyDetail`, sort disponibles primero en frontend y backend, CORS `PATCH` fix, endpoint `PATCH /admin/properties/{id}/status`. Branch `feat/toggleOnOff`. |
